...
export function apply(ctx, config = {}) {
  const baseCfg = { ...DEFAULT_CFG, ...config };
  ctx.inject?.(["settings"], (sctx) => {
    const scope = sctx.settings.register(NS, Config, { base: baseCfg });
    // Read the live registered value per request: settings-file edits
    // (repoOwner/repoName/branch/…) hot-publish and must take effect
    // without a host restart (schema defaults + base + user layer).
    // Fall back to the apply-time snapshot if no live scope is available.
    registerGateway(ctx, () => scope?.get?.() ?? baseCfg);
  });
}
